  ...
  if name.find('_JAAS_PLACEHOLDER') > -1:
    if security_enabled:
      if name.find('Nimbus_JVM') > -1:
        ...
        return name.replace('_JAAS_PLACEHOLDER', '-Dzookeeper.sasl.client.username=' + params.zookeeper_principal_primary + ' -Djava.security.auth.login.config=' + conf_dir + '/storm_jaas.conf')
    else:
