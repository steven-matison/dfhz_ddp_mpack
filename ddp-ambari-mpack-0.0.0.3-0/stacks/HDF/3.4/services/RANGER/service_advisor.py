...
class HDF34RANGERServiceAdvisor(service_advisor.RangerServiceAdvisor):

  def __init__(self, *args, **kwargs):
    self.as_super = super(HDF34RANGERServiceAdvisor, self)
    self.as_super.__init__(*args, **kwargs)

  def getServiceConfigurationRecommendations(self, configurations, clusterData, services, hosts):
    super(HDF34RANGERServiceAdvisor, self).getServiceConfigurationRecommendations(configurations, clusterData, services, hosts)

    recommender = HDF34RangerRecommender()
    recommender.recommendRangerConfigurationsFromHDF34(configurations, clusterData, services, hosts)

class HDF34RangerRecommender(service_advisor.ServiceAdvisor):
  """
  Ranger Recommender suggests properties when adding the service for the first time or modifying configs via the UI.
  """

  def __init__(self, *args, **kwargs):
    self.as_super = super(HDF34RangerRecommender, self)
    self.as_super.__init__(*args, **kwargs)

  def recommendRangerConfigurationsFromHDF34(self, configurations, clusterData, services, hosts):
