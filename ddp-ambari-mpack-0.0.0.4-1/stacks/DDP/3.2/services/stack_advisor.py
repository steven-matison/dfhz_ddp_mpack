...
class DDP32StackAdvisor(DefaultStackAdvisor):

  def getComponentLayoutValidations(self, services, hosts):
    """Returns array of Validation objects about issues with hostnames components assigned to"""
    items = super(DDP32StackAdvisor, self).getComponentLayoutValidations(services, hosts)

    # Use a set for fast lookup
    hostsSet =  set(super(DDP32StackAdvisor, self).getActiveHosts([host["Hosts"] for host in hosts["items"]]))  #[host["Hosts"]["host_name"] for host in hosts["items"]]
  ...
  def recommendRangerConfigurations(self, configurations, clusterData, services, hosts):
    ...
    # Check if http is disabled. For DDP this can be checked in ranger-admin-site/ranger.service.http.enabled
  ...
  def recommendAmsConfigurations(self, configurations, clusterData, services, hosts):
    ...
    if operatingMode == "distributed":
      dn_hosts = self.getComponentHostNames(services, "DDPS", "DATANODE")
  ...
  def validateAmsHbaseSiteConfigurations(self, properties, recommendedDefaults, configurations, services, hosts):
    ...
    if op_mode == "distributed" and is_local_root_dir:
      rootdir_item = self.getWarnItem("In distributed mode hbase.rootdir should point to DDPS.")
    elif op_mode == "embedded":
      if distributed.lower() == "false" and hbase_rootdir.startswith('/') or hbase_rootdir.startswith("hdfs://"):
        rootdir_item = self.getWarnItem("In embedded mode hbase.rootdir cannot point to schemaless values or DDPS, "
        ...
        if host["Hosts"]["host_name"] == collectorHostName:
              ...
              for dfs_datadir in dfs_datadirs:
                ...
                if dfs_datadir_mountpoint == hbase_rootdir_mountpoint:
                  item = self.getWarnItem("Consider not using {0} partition for storing metrics data. "
                                          "{0} is already used by datanode to store DDPS data".format(hbase_rootdir_mountpoint))
          ...
          # If no local DN in distributed mode
            ...
            item = self.getWarnItem("It's recommended to install Datanode component on {0} "
                                    "to speed up IO operations between DDPS and Metrics "
