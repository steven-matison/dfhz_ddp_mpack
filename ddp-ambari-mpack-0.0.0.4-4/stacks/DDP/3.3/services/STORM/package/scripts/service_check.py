...
class ServiceCheckDefault(ServiceCheck):
  def service_check(self, env):
    ...
    if params.nimbus_seeds_supported:
      ...
      cmd = format("storm jar /tmp/wordCount.jar storm.starter.WordCountTopology WordCount{unique} -c client.jartransformer.class=org.apache.storm.hack.StormShadeTransformer")
    elif params.nimbus_host is not None:
      cmd = format("storm jar /tmp/wordCount.jar storm.starter.WordCountTopology WordCount{unique} -c client.jartransformer.class=org.apache.storm.hack.StormShadeTransformer -c nimbus.host={nimbus_host}")
